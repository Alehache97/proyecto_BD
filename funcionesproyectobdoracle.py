...
def connectoracle(host, usuario, contraseña):
    try:
        oracledb.init_oracle_client(lib_dir=r"C:\\Users\\aleja\\Documents\\1ASIR\\Gestión de Bases de Datos\\T4 DML\\instantclient-basic-windows.x64-11.2.0.4.0\\instantclient_11_2")  # Especifica la ruta al directorio Instant Client
        db = oracledb.connect(user=usuario, password=contraseña, dsn=host, mode=oracledb.SYSDBA)  # Especifica el modo Thick y otras opciones, si es necesario
    except oracledb.DatabaseError as e:
        error, = e.args
        print("Error de conexión a la base de datos:", error.message)
        sys.exit(1)
    print("Conexión correcta en Oracle")
    return db
